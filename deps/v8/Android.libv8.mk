...
NODE-GEN1 := $(intermediates)/libraries.cc
$(NODE-GEN1): SCRIPT := $(intermediates)/js2c.py
$(NODE-GEN1): $(LOCAL_JS_LIBRARY_FILES) $(JS2C_PY)
	@echo "Generating libraries.cc"
	@mkdir -p $(dir $@)
	python $(SCRIPT) $(NODE-GEN1) CORE off $(LOCAL_JS_LIBRARY_FILES)
V8_GENERATED_LIBRARIES := $(intermediates)/libraries.cc

# Generate experimental-libraries.cc
NODE-GEN2 := $(intermediates)/experimental-libraries.cc
$(NODE-GEN2): SCRIPT := $(intermediates)/js2c.py
$(NODE-GEN2): $(LOCAL_JS_EXPERIMENTAL_LIBRARY_FILES) $(JS2C_PY)
	@echo "Generating experimental-libraries.cc"
	@mkdir -p $(dir $@)
	python $(SCRIPT) $(NODE-GEN2) EXPERIMENTAL off $(LOCAL_JS_EXPERIMENTAL_LIBRARY_FILES)
...
NODE_SNAP_GEN := $(intermediates)/snapshot.cc
NODE_MKSNAPSHOT := $(HOST_OUT_EXECUTABLES)/node-mksnapshot.$(TARGET_ARCH)
$(NODE_SNAP_GEN): PRIVATE_CUSTOM_TOOL = $(NODE_MKSNAPSHOT) --logfile $(intermediates)/v8.log $(NODE_SNAP_GEN)
$(NODE_SNAP_GEN): $(MKSNAPSHOT)
	$(transform-generated-source)
LOCAL_GENERATED_SOURCES += $(NODE_SNAP_GEN)
else
LOCAL_SRC_FILES += \
